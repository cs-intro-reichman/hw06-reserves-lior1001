...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		imageOut = scaled(eyes, 4, 4);
		Color[][] blend = blend(tinypic, imageOut, 0.25);
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for(int i=0; i<image.length; i++) {
			//go over the columns
			for(int j=0; j<image[0].length; j++) {
				flippedHorizontal[i][j] = image[i][image[0].length-1-j];
			}
		}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i=0; i<image[0].length; i++) {
			...
			for(int j=0; j<image.length; j++) {
				flippedVertical[j][i] = image[image.length-1-j][i];
			}
		}
	...
	public static Color luminance(Color pixel) {
		...
		Color lumCol = new Color(lum, lum, lum);
		return lumCol;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i=0; i<height; i++) {
			for(int j=0; j<width; j++) {
				scaledPic[i][j] = image[i*h0/height][j*w0/width];
			}
		}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color blendCol = new Color(newR, newG, newB);
		return blendCol;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		//if the images are not the same dimensions, scale the target image
		if(source.length != target.length || source[0].length != target[0].length) {
			target = scaled(target, source[0].length, source.length);
		}
		Color[][] currBlend = new Color[source.length][source[0].length];
		for(int i=0; i<n; i++) {
			currBlend = blend(source, target, (n-i)/n);
			display(currBlend);
			StdDraw.pause(500);
		}
	}
